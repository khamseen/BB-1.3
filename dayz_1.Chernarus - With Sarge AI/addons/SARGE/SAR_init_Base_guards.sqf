...
diag_log format["count _flagPoles: %1", (count _flagPoles)];
	{
		...
		_sizeX = 50; 
		_sizeY = 50;
		if (count _sizeOfBase >= 10) then { _rifleMen = 2; _snipers = 1; _sizeX = 75; _sizeY = 75};
		if (count _sizeOfBase >= 20) then { _rifleMen = 3; _snipers = 1; _sizeX = 100; _sizeY = 100};
		if (count _sizeOfBase >= 30) then { _rifleMen = 3; _snipers = 2; _sizeX = 150; _sizeY = 150};
